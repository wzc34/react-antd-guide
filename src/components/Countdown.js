...
export default class Countdown extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (date > 0) this.interval = setInterval(this.tick, this.props.intervalDelay);
  ...
  render() {
    ...
    if (this.state.completed && this.props.children) {
      ...
      const { hours, minutes, seconds } = formattedDelta;
      let span
      if(hours!=='00'){
        span = (<span>{hours}:{minutes}:{seconds}</span>)
      }else{
        // span = (<span>{minutes}:{seconds}</span>)
        span = (<span>{seconds === '00' ? 60 : seconds}秒后重新发送</span>)
      }

      if(seconds==='00' && minutes==='00'){
        span = (<span>{'重新获取验证码'}</span>)
      }
      return span
    }
  }
